...
export async function initBilling() {
  ...
  try {
    ...
    store.register(CARROT_PACKAGES.map(p => ({
      id: p.id,
      type: ProductType.CONSUMABLE,
      platform: Platform.GOOGLE_PLAY,
    })));

    // Onaylanan siparişleri işle → havuç ekle → tüket
    store.when()
...
export async function purchase(productId) {
  ...
  if (!pkg) return { ok: false, reason: 'Ürün bulunamadı' };

  // Web / geliştirme: gerçek SDK yok → simüle et
  if (!isNative || !CDV()) {
    if (onGrant) onGrant(pkg.id, pkg.carrots);
